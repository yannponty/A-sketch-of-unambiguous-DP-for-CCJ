...
\textbf{HJ-} I am not yet clear with the ``*'' notation. If it is regular-expression-like, then does it mean in $\PKnone\constrIO$ we can have more than one iteration to get from a $l$ state to an $O$ state? \YP{Yup, that is my understanding or, more precisely, $\PKnone\constrIO$ means {\em a (possibly empty) sequence of 'anything but $L$eft' base pairs, ultimately followed by an $O$utside base pair}.}
...
%\end{equation}
\begin{GRule}
  \GRgapfragDeco{$\PKnone1\constr$}{}{}
  \GRdecompTo
  \GRgapfragWDecompJ{$\PKnone\constr$}{}{}{$\WPnone$}
\end{GRule}
\qquad
\begin{GRule}
  \GRgapfragDeco{$\PKnone2\constr$}{}{}
  \GRdecompTo
  \GRgapfragWDecompK{$i\PKnone1\constr$}{}{}{$\WPnone$}
\end{GRule}
