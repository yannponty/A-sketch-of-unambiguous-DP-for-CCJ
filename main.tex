...
$P_{fromX}$-fragments are required to change the band; first, we need to unambiguously allow recursive substructure, then $P''_{fromX}$ must guarantee unambiguous band change. For the unconstrained $P_O$, the recursion to the unconstrained $P_\text{fromO}$ works essentially like in the original recursions. 
\begin{equation}
\includegraphics{KnottyPF/recursionPfromX}
\end{equation}
For the constrained case, we still need to work out all the details.
