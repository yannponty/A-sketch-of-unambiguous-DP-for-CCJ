...
\end{itemize}
\end{definition}
\YP{This concept seems a bit loose IMO, ie both $\{A,B,C\}$ and $\{A,B,D\}$ seem to fulfill the definition of band in

{\centering\tt%
ABCD...dbca\\
(([(...))])\\}

Probably nitpicking here, but want to make sure we understand each other (+ uniqueness of decomposition is important for future unambiguity proofs)...}
...
\textbf{HJ-} I am not yet clear with the ``*'' notation. If it is regular-expression-like, then does it mean in $PK_{l*O}$ we can have more than one iteration to get from a $l$ state to an $O$ state? \YP{Yup, that is my understanding or, more precisely, $PK_{l*O}$ means {\em a (possibly empty) sequence of 'anything but $L$eft' base pairs, ultimately followed by an $O$utside base pair}.}
...
where, generically, fragments $PK_{\star}^1$ and $PK_{\star}^2$ are decomposed into fragments $PK_{\star}$ and $WP$ to account unambiguously for recursive substructure:
...
%\begin{align*}
%&\begin{GRule}
%  \GRgapfraglO[outer arc/.style={dotted arc}]{$P_{\text{fromO}}$} 
%  \GRdecompTo
%  \GRwfrag[itype/.style=fix]{$W\!P$}\GRconcat
%  \GRgapfrag[left arc/.style={solid arc},itype/.style=free]{$P_{\text{L}}$}
%  \GRor
%  \GRgapfrag[right arc/.style={solid arc},ltype/.style=free]{$P_{\text{R}}$}
%  \GRconcat\GRwfrag[ltype/.style=fix]{$W\!P$}
%\end{GRule}\\
%&\begin{GRule}
%  \GRgapfraglO[right arc/.style={dotted arc}]{$P_{\text{fromR,l*O}}$} 
...
%\end{GRule}\\
%&\begin{GRule}
%  \GRgapfraglO[inner arc/.style={dotted arc}]{$P_{\text{fromM}}$} 
%  \GRdecompTo
%  \GRgapfraglO[right arc/.style={solid arc},ktype/.style=free]{}\GRseclabel{$P_{\text{R,l*O}}$}
%  \GRconcatWBefore{k}\GRwfrag[itype/.style=fix]{$W\!P$}\GRrestorelast
%\end{GRule}
%\end{align*}
