...
\begin{definition}[Weakly closed region] 
Region $[i,l]$ is weakly closed if all bases in $[i,l]$ are either unpaired or paired with other bases in $[i,l]$.  
\end{definition}
...
% HJ- Definition from CCJ paper
\begin{definition}[Three-Group-of-Band (TGB) structure]
If $R$ is a secondary structure for a sequence of length
$n$, let $R_{i,l}$ be the subset of base pairs of $R$ whose endpoints are in region $[i,l]$, and let $R_{[i, j] \cup [k,l]}$ be the subset of $R$
whose endpoints are in $[i,j] \cup [k.l]$.  (Thus, $R = R_{1,n}$).
\begin{enumerate}
\item
$R_{i \cup j}{k \cup l}$ is a {\em TGB structure} if either
\begin{enumerate}
\item
$[i,j] \cup [k,l]$ is a gapped region,
$R_{[i, j] \cup [k, l]}$ contains base pair(s) $i.l$ and $j.k$, any
base pair of $R_{[i, j]\cup[k, l]}$ 
which spans the gapped region $[i,j] \cup [k,l]$
does not cross any other base pair of $R_{[i, j]\cup[k, l]}$, and any nested substructures $R_{[i, j]\cup[k, l]}$,
in weakly closed regions $[i,j]$ or $[k,l]$ are CCJ
structures.

\item
$R_{[i, j]\cup[k, l]}$ can be decomposed into a
CCJ structure and a TGB structure in one of the following ways, for some $d$:
...
\noalign{\smallskip} 
\hline \hline
$i< d \le j$    & $R_{i, d-1}$  & $R_{[d, j] \cup [k, l]}$ \\[.01in]
$i \le d< j$    & $R_{d+1, j}$  & $R_{[i, d] \cup[k, l]}$ \\[.01in]
$k< d \le l$    & $R_{k, d-1}$  & $R_{[i,j] \cup [d, l]}$ \\[.01in]
$k \le d< l$    & $R_{d+1, l}$  & $R_{[i, j] \cup [k, d]}$ \\[.01in]
\noalign{\smallskip} 
\hline
\end{tabular}
\end{center}
...
\item
$R_{[i,j] \cup [k,l]}$ can be decomposed into a band and a TGB structure in 
one of the following ways, for some $d$ and $e$:

\begin{center}
\begin{tabular}{ccrlc} \hline
\noalign{\smallskip} 
range of $d$ & range of $e$ & \multicolumn{2}{c}{band borders}    & TGB structure      \\ 
\noalign{\smallskip} 

\hline \hline
$i\le d<j$   & $i<e \le j$  &  $i.j$     & $d.e$    & $R_{[d+1,e-1] \cup [k,l]}$ \\[.01in]
$i\le d<j$   & $k<e \le l$  &  $i.l$     & $d.e$    & $R_{[d+1,j] \cup [k,e-1]}$ \\[.01in]
$i<d\le j$   & $k \le e<l$  &  $d.e$     & $j.k$    & $R_{[i,d-1] \cup [e+1,l]}$ \\[.01in]
$k\le d<l$   & $k<e \le l$  &  $k.l$     & $d.e$    & $R_{[i,j] \cup [d+1,e-1]}$ \\
\noalign{\smallskip} 
\hline
\end{tabular}
\end{center}
...
\begin{definition}[CCJ pseudoknot]
$R_{i,l}$ is a {\em CCJ pseudoknot} if it is the union of
two TGB structures $R_{[i,j]\cup[d+1, k]}$ and $R_{[j+1,d]\cup[k+1, l]}$.
\end{definition}

\begin{definition}[CCJ structure]
Finally, $R_{i,l}$ is a {\em CCJ structure} if
\begin{enumerate}
\item
$R_{i,l}$ is empty, or

\item
$i.l \in R_{i,l}$ and $R_{i+1,l-1}$ is a CCJ structure, or

\item
for some $k \in [i,l-1]$, $R_{i,l} = R_{i,k} \cup R_{k+1,l}$
and both $R_{i,k}$ and $R_{k+1,l}$ are CCJ structures, or

\item
$R{i,l}$ is a CCJ pseudoknot.
\end{enumerate}
\end{definition}
...
%HJ- I think we need to add definition of each recursion here as well.
\textbf{HJ-} The W-rule over region $[i,l]$ describes decomposition of a W-fragment to find the minimum free energy structure that is not nested in a substructure. 
Similar changes disambiguate the WB and WP recursions, which respectively are minimum free energy structures nested in a band or a pseudoloop. 
V-fragment represents the structure R over region $[i,l]$ where i is paired with l; in this case R can be a hairpin loop, an internal loop or a multiloop.
...
\begin{enumerate}
\item either $i.i'$ and $l'.l$ directly cross each other 
...
We sketch the required base pairs (i.e. $i.i'$ and $l'.l$) in red; note that in all cases there have to be base pairs that span the region of the gapped fragments. In the cases, where such base pairs are required to connect the left and right end by a chain of crossing base pairs, we indicate them by a blue base pair.
...
\noindent \textbf{HJ-} I checked CCJ algorithm again, and it was an error in my thesis to remove transition from M to O case. In the published version there is a transition from M to O but not the other way.
...
\item $\P_4 = \{R\in \P \mid \exists i',l',p,p',q, q': i.i'\in R, p.p'\in R, q.q'\in R, l'.l\in R, p<i'<q<p'<l'<q'\}$.
\end{enumerate}
...
While $PK$-fragments are used in the decomposition of $P$-fragments, their only purpose is to decompose $P$-fragments into a $P_X$ and a $P_Y$-fragment ($X,Y\in\{L,R,M,O\}$), while unambigously allowing recursive substructure.
...
The structures of a $\POnone$-fragment, 
i.e. the $P_X$-structures, are the $PK$-structures $R$ over the fragment's region $[i,j]\cup[k,l]$, where
\begin{itemize}
\item $i.l \in R$
